...
# BlendshapeWeightMatrix = uploadBSWeightMatrix()
# setupBlendshapes();
# useIDsBlendShapeWeights(1,1)

def uploadBSWeightMatrix(*args):
	# filePath = "bs_weights.mat"
	filePath = "weightMatrix.mat"
	print 'Loading 3DMM: ' + filePath
	matData = spio.loadmat(filePath)
	weights = matData['weightMatrix']
...
def setupBlendshapes(FACE_ID,envName,tIDs,oN,*args):
	...
	#filePath = "bs_weights.mat"
	filePath = "Blendshapes/smile/weightFMatrix.mat"
	print 'Loading 3DMM: ' + filePath
	matData = spio.loadmat(filePath)
	weights = matData['weightFMatrix']
	BlendshapeWeightMatrix = np.transpose(weights)
...
def create18Faces(startID,*args):
	...
	for face in range(1,19):
		...
		mel.eval('blendShape;')
		randomTarget = randomTargetList[face-1] + 1
		print 'Face ' + str(face) + ' using Target smile ' + str(randomTarget)
		useIDsBlendShapeWeights(face+1,randomTarget,startingID)
	...
	randomTargetList = randomTargetList[18:]
	# createGeneUI()
...
def generateNextGen(bF,cBvals,interFaceCond,*args):
	...
	for i in range(1,19):
		if i==1:
			...
			numChanges = 3
			chosenPCs = random.sample(range(Num_BlendShapes),numChanges)
			...
			try:
				randomSmile = randomTargetList[1]
			except IndexError:
				print 'WARNING'
				randomTargetList = setUpRandomTarget()
			randomFace = randomTargetList[0]
			randomTargetList = randomTargetList[1:]
			useIDsBlendShapeWeights(i+1,randomFace,startingID)
			numChanges = 3
			chosenPCs = random.sample(range(Num_BlendShapes),numChanges)
			ranFace = mutateBlendshapes(i,0.5,chosenPCs)
			setBlendShapePCs(i,ranFace,startingID)
			print 'Face ' + str(i) + ' spawned as ID:' + str(randomFace) + '... BSs chosen:' + str(chosenPCs)
		else:
			numChanges = 5
			chosenPCs = random.sample(range(Num_BlendShapes),numChanges)
...
def saveFace(bF,*args):
	...
	fileName = 'FACE' + str(startingID) + '_sampleSolvedSmiles_' + outName + '.mat'
	spio.savemat(fileName, {"BlendshapeWeights":eliteFace})

def setUpRandomTarget(*args):

	rTList = np.random.permutation(64)
	newIDs = [x - 1 for x in testIDs]
	rTList = np.setdiff1d(rTList,newIDs)
