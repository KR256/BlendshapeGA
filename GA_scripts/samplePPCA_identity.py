...
def setupBlendshapes(FACE_ID,envName,tIDs,oN,*args):
	...
	generation = 0
	os.chdir(owd)
	filePath = "Resources/PPCA_neutral.mat"
	...
	os.chdir("Blendshapes/neutral/identityPPCA")
...
def generateNextGen(bF,cBvals,interFaceCond,*args):
	...
	for i in range(1,19):
		if i==1:
			...
			print 'Face 3 chosen as exageration of ELITE face'
			print exagFaceScore
			print exagFaceMesh
		# elif i<(len(chosenFaces2)+len(chosenCombos)+3):
			...
			chosenTemp = random.choice(chosenCombos)
			# j = i - (len(chosenFaces2)+4)
			# chosenTemp = chosenCombos[j]
			f1 = chosenTemp[0]
			f2 = chosenTemp[1]
			...
			# print 'cFace:'
			# print cFace
			ranFace = mutateBlendshapes(f1,0.5,chosenPCs,f1,cFace)
	...
	# global groundTruth
	# global ppcaIdentityBSdeltas

	# smileBSscores = createFaceBSLog()

	# faceVec = blendshapesToVec(smileBSscores[0],ppcaIdentityBSdeltas)
	# print faceVec
...
def saveFace(bF,*args):
	...
	fileName2 = 'SmileLog_FACE' + str(startingID) + '_ppcaIdentityBSv_' + str(outName) + '.mat'
	fileName3 = 'SmileChosenLog_FACE' + str(startingID) + '_ppcaIdentityBSv_' + str(outName) + '.mat'
	...
	fileName = 'FACE' + str(startingID) + '_samplePPCAIdentityBSv_' + outName + '.mat'
...
def automatedEvolution(NUM_GENERATIONS,*args):
	...
	filePath = "Blendshapes/neutral/identityPPCA/ppcaIdentityBSdeltas.mat"
	matData = spio.loadmat(filePath)
	ppcaIdentityBSdeltas = matData['ppcaIdentityBSdeltas']
	...
	for gen in range(NUM_GENERATIONS):
		...
		genResults = compareWithGroundTruth(groundTruth,ppcaIdentityBSdeltas,smileBSscores)
